...
function json2radio( jsonstring ) {
	var jsn = jsonstring.replace( /'/g, '"' );
	var jsn = JSON.parse( jsn );
	var radio = '';
	for ( var key in jsn ) {
		radio += '<input type="radio" name="inforadio" value="'+ jsn[ key ] +'"> '+ key +'<br>';
	}
	return radio
}
function json2checkbox( jsonstring ) {
	var jsn = jsonstring.replace( /'/g, '"' );
	var jsn = JSON.parse( jsn );
	var checkbox = '';
	for ( var key in jsn ) {
		checkbox += '<input type="checkbox" value="'+ jsn[ key ] +'"> '+ key +'<br>';
	}
	return checkbox
}
function json2selectbox( jsonstring ) {
	var jsn = jsonstring.replace( /'/g, '"' );
	var jsn = JSON.parse( jsn );
	var selectbox = '<select>';
	for ( var key in jsn ) {
		selectbox += '<option value="'+ jsn[ key ] +'"> '+ key +'</option>';
	}
