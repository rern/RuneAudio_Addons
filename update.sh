...
if [[ -e /srv/http/assets/img/addons/addons.png ]]; then
	file=/srv/http/assets/js/addonsinfo.js
	[[ $( grep cancelbutton $file ) ]] && wget -qN --no-check-certificate https://github.com/rern/RuneAudio_Addons/raw/UPDATE$file -O $file
else
